            ...
            <div className="event-listings">
                {filteredEventPostings.map(event => (
                    <div key={event._id} className="event-posting">
                        <div className="event-title-container" onClick={() => toggleExpandEvent(event._id)}>
                            <h3>{event.eventName}</h3>
                            <span className={`arrow-icon ${expandedEventId === event._id ? 'expanded' : ''}`}>&#9660;</span>
                        </div>
                        ...
                        {expandedEventId === event._id && (
                            <div className="event-details">
                                ...
                                {event.registrationRequired && (
                                    <button className="register-button" onClick={() => navigateToRegistration(event.registrationLink)}>Register</button>
                                )}
                            </div>
                        )}
                    </div>
